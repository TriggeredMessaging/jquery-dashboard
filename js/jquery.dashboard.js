...
(function($) { // Create closure.
    ...
    $.fn.dashboard = function(options) {
        ...
        function updateColumnEmptyStates() {
            $('.' + opts.columnClass).each(function(i, o) {
                updateColumnEmptyState(o);
            })
        }

        function updateColumnEmptyState(column) {
            dashboard.log('Checking if column is now empty.', 1);
            column = $(column);
            var childLength = column.children().length;
            if(childLength == 0) {
                dashboard.log('adding the empty text to the column', 1);
                column.html('<div class="emptycolumn">' + opts.emptyColumnHtml + '</div>');
            } else {
                if(childLength == 2) {
        ...
        function widget(widget) {
            ...
            widget.remove = function() {
                ...
                // Delete the reference to the existing widget
            ...
            widget.serialize = function() {
                ...
                if(widget.url) r.url = widget.url;
                if(widget.column) r.column = widget.column;
                if(widget.editurl) r.editurl = widget.editurl;
                if(typeof widget.open !== 'undefined') r.open = widget.open;

                //var r = '{"title" : "' + widget.open + '", "id" : "' + widget.id + '", "column" : "' + widget.column + '","editurl" : "' + widget.editurl + '","open" : ' + widget.open + ',"url" : "' + widget.url + '"';

                if(typeof widget.metadata != 'undefined') {
                    r.metadata = widget.metadata;
                    /*r+= ',"metadata":{'
        ...
        dashboard.element.live('addWidgetDialogSelectCategory', function(e, obj) {
            ...
            $.getJSON(url, {"cache": true}, function(json) {
                ...
                $.each(items, function(i, item) {
                    ...
                    if(dashboard.widgets[item.id]) {
                        html = tmpl($('#' + addOpts.widgetTemplateUsed).html(), item);
                    } else {
                        html = tmpl($('#' + addOpts.widgetTemplate).html(), item);
                    }
    ...
    $.fn.dashboard.defaults = {
        ...
        addWidgetSettings: {
            ...
            widgetTemplateUsed: 'addwidgettemplateused'
        },
        editLayoutSettings: {
            dialogId: 'editLayout',
